class_name DroughtEventEffectService
extends RefCounted


func apply(world: World, event: NaturalEventInstance, rules: DroughtEventRules) -> void:
	...
	if affected_cells.is_empty():
		print("[SICCITA] anno=%d centro=(%d,%d) nessuna cella di terra colpita" % [
	...
	for pos in affected_cells:
		...
		if state == null:
			continue
		state.register_growth_bonus(GameTypes.NaturalEventType.DROUGHT, rules.post_event_growth_multiplier, duration)

	print("[SICCITA] anno=%d centro=(%d,%d) intensita=%d raggio=%d celle_colpite=%d moltiplicatore_crescita=%.2f durata=%d" % [
