...
@onready var species_filter_container: HBoxContainer = $MarginContainer/VBoxContainer/TabContainer/FaunaTab2/FaunaContent/SpeciesFilterContainer
...
# Ricostruita ad ogni refresh insieme all'elenco sotto (stesso pattern "queue_free + rebuild" già
# usato da population_groups_container/fish_summary_container in questo file) — pochi pulsanti,
# nessun bisogno di un aggiornamento incrementale.
func _rebuild_species_filter_row(world: World) -> void:
	...
	species_filter_container.add_child(all_button)
...
# rabbit/deer: stessi colori del renderer sul campo (AnimalGroupRenderer), un solo posto li
# definisce. Specie future senza un colore dedicato: derivato deterministicamente dal nome
# (stesso hash ad ogni refresh, quindi lo stesso colore ogni volta per la stessa specie) invece
# di un colore casuale che cambierebbe ad ogni apertura del pannello.
